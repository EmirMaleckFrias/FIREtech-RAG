...
describe("un PDF escaneado", () => {
  test("no tiene texto propio: sin OCR falla con el mensaje de siempre", async () => {
    const { paginas, imagenes } = await extraerLineas(ESCANEADO);
    expect(paginas).toHaveLength(1);
    expect(textoDePagina(paginas[0])).toBeLessThan(40);
    // Sin pedirlas, las imágenes no se extraen: es lo que ahorra el trabajo
    // en un artículo normal con figuras.
    expect(imagenes.size).toBe(0);
